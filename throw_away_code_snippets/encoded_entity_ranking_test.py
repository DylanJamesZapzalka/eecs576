...
for i in tqdm(range(len(question_embeddings)), desc='Evaluating over each question/answer'):
    ...
    scores, retrieved_examples = wiki_dataset.get_nearest_examples('embeddings', question_embedding.cpu().numpy(), k=100)
    ...
    for retrieved_example in retrieved_examples:
        ...
        for entity in entities:
            # Get entity tokens
            entity_tokens = ctx_tokenizer(entity.get_label() ,max_length=12,truncation=True,padding='max_length',return_tensors='pt').to(device)
            entity_embedding = ctx_encoder(**entity_tokens)
            entity_embedding = entity_embedding.pooler_output
            entity_embedding = entity_embedding.detach()
            score = torch.squeeze(entity_embedding).dot(torch.squeeze(question_embedding))
