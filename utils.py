...
def get_data(retrieved_examples, answers, nlp, number_of_links=3):

    # Get node feature vectors
    x = torch.tensor(retrieved_examples['embeddings']).to(device)
    edge_index = get_edge_index(retrieved_examples['text'], nlp, number_of_links)
    y = get_labels(retrieved_examples['text'], answers)
    data = Data(x=x, edge_index=edge_index.t().contiguous())
    return data, y


def get_edge_index(retrieved_examples, nlp, number_of_links):
    ...
    adjacency_matrix = torch.zeros((100, 100))
    edge_index = []
